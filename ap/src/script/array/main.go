...
func reservationJoinUser() {
	...
	on
	reservations.user_id = users.id`).Rows()
...
func reservationScanUser() {
	...
	sqlHandler.Debug().Table("reservations").Select("reservations.id,reservations.starts_at, reservations.user_id, users.user_name").Joins("left join users on reservations.user_id = users.id").Scan(&results)
